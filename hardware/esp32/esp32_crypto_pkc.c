...
void curve448Mul(uint32_t *r, const uint32_t *a, const uint32_t *b)
{
   ...
   DPORT_REG_WRITE(RSA_INTERRUPT_REG, 1);
   //Set mode register
   DPORT_REG_WRITE(RSA_MULT_MODE_REG, 9);
   ...
   //The second operand must be written to the base address of the
   //RSA_Z_MEM memory plus the address offset 64
   for(i = 0; i < 14; i++)
   {
      DPORT_REG_WRITE(RSA_MEM_Z_BLOCK_BASE + 64 + i * 4, b[i]);
   }

   //Pad the second operand with zeroes
   DPORT_REG_WRITE(RSA_MEM_Z_BLOCK_BASE + 120, 0);
   DPORT_REG_WRITE(RSA_MEM_Z_BLOCK_BASE + 124, 0);
