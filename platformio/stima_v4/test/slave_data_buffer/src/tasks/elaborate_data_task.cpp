...
void ElaborateDataTask::make_report (bool is_init, uint16_t report_time_s, uint8_t observation_time_s) {
  ...
  rmapdata_t sum_battery_charge = 0;
  rmapdata_t sum_battery_voltage = 0;
  rmapdata_t sum_battery_current = 0;
  rmapdata_t sum_input_voltage = 0;
  rmapdata_t sum_input_current = 0;

  rmapdata_t sum_battery_charge_quality = 0;
  rmapdata_t sum_battery_voltage_quality = 0;
  rmapdata_t sum_battery_current_quality = 0;
  rmapdata_t sum_input_voltage_quality = 0;
  rmapdata_t sum_input_current_quality = 0;
  ...
  float error_battery_charge_per_o = 0;
  ...
  static rmapdata_t avg_battery_charge_quality_o;
  ...
  if (is_init) {
    ...
    avg_battery_charge_quality_o = 0;
    avg_battery_voltage_o = 0;
    avg_battery_voltage_quality_o = 0;
    avg_battery_current_o = 0;
    avg_battery_current_quality_o = 0;
    avg_input_voltage_o = 0;
    avg_input_voltage_quality_o = 0;
    avg_input_current_o = 0;
    avg_input_current_quality_o = 0;
  }
  ...
  // Reset Pointer Back Read to last data avaiable (last data add to buffer)
  ...
  // Align all data buffer to one (in case read data when push complete list of value isn't completed)
  // For es. if we have add battery_charge battery_current but not again input_voltage... ecc... for same sampèle time
  ...
  // flush all data that is not aligned (perform a readback with unused value buf need for align data)
  ...
  if (report_time_s && observation_time_s)
  {
    ...
    // TODO: NO for (uint16_t i = 0; i < samples_count; i++)
    for (uint16_t i = 0; i < report_sample_count; i++)
    {
      ...
      {
        sum_battery_charge_quality += (rmapdata_t)(checkMppt(battery_charge));

        if (ISVALID_RMAPDATA(battery_charge))
        {
          valid_count_battery_charge++;
          sum_battery_charge += (rmapdata_t)(battery_charge);
        }
        else
        {
      ...
      if (!measures_maintenance)
      {
        sum_battery_voltage_quality += (rmapdata_t)(checkMppt(battery_voltage));

        if (ISVALID_RMAPDATA(battery_voltage))
        {
          valid_count_battery_voltage++;
          sum_battery_voltage += (rmapdata_t)(battery_voltage);
        }
        else
        {
      ...
      if (!measures_maintenance)
      {
        sum_battery_current_quality += (rmapdata_t)(checkMppt(battery_current));

        if (ISVALID_RMAPDATA(battery_current))
        {
          valid_count_battery_current++;
          sum_battery_current += (rmapdata_t)(battery_current);
        }
        else
        {
      ...
      if (!measures_maintenance)
      {
        sum_input_voltage_quality += (rmapdata_t)(checkMppt(input_voltage));

        if (ISVALID_RMAPDATA(input_voltage))
        {
          valid_count_input_voltage++;
          sum_input_voltage += (rmapdata_t)(input_voltage);
        }
        else
        {
      ...
      if (!measures_maintenance)
      {
        sum_input_current_quality += (rmapdata_t)(checkMppt(input_current));

        if (ISVALID_RMAPDATA(input_current))
        {
          valid_count_input_current++;
          sum_input_current += (rmapdata_t)(input_current);
        }
        else
        {
    ...
    // TOUP: ?! Acquisire solo i sample richiesti in bufferBack necessari al report... non tutto il buffer
    // TODO: ?! Riempire con i dati calcolati semplici a prima lettura...

    // report.avg_battery_charge = avg_battery_charge_o;
    // report.avg_battery_charge_quality = avg_battery_charge_quality_o;
    if(valid_count_battery_charge > (report_sample_count/2)) {
      report.avg_battery_charge = sum_battery_charge / valid_count_battery_charge;
    }
    report.avg_battery_charge_quality = sum_battery_charge_quality / report_sample_count;
