...
void ModemTask::Run() {
  ...
  while (true)
  {
    ...
    case MODEM_STATE_INIT:
      ...
      if(is_error) {
        Serial.println("// ***** TEST Initialize NetPPP Cyclone Interface FAIL");
        // ***** TEST Initialize NetPPP Cyclone Interface FAIL
      } else {
        Serial.println("// ***** TEST Initialize NetPPP Cyclone Interface OK");
        // ***** TEST Initialize NetPPP Cyclone Interface OK
      }
    ...
    case MODEM_STATE_WAIT_NET_EVENT:
      ...
      // ***** TEST Initialize NetPPP and GSM Interface (WAIT CONNECTION START COMMAND QUEUE)
      Serial.println("// ***** TEST Initialize NetPPP and GSM Interface (WAIT CONNECTION START COMMAND QUEUE)");
      ...
      if (param.connectionRequestQueue->Peek(&connection_request, portMAX_DELAY))
      {
        ...
        if (connection_request.do_connect)
        {
          ...
          // ***** TEST Queue command to Start Connection
          Serial.println("// ***** TEST Queue command to Start Connection");
    ...
    case MODEM_STATE_SWITCH_ON:
      ...
      {
        // ***** TEST GSM Power ON OK
        Serial.println("// ***** TEST GSM Power ON OK");
        state = MODEM_STATE_SETUP;
        TRACE_VERBOSE_F(F("MODEM_STATE_SWITCH_ON -> MODEM_STATE_SETUP\r\n"));
      }
      else if (status == SIM7600_ERROR)
      {
        // ***** TEST GSM Power ON FAILED
        Serial.println("// ***** TEST GSM Power ON FAILED");
        is_error = true;
    ...
    case MODEM_STATE_SETUP:
      ...
      if (status == SIM7600_OK)
      {
        Serial.println("// ***** TEST Initialize List AT COMMAND GSM Interface");
        // ***** TEST Initialize List AT COMMAND GSM Interface
      ...
      {
        Serial.println("// ***** TEST FAIL Initialize List AT COMMAND GSM Interface");
        // ***** TEST FAIL Initialize List AT COMMAND GSM Interface
        is_error = true;
    ...
    case MODEM_STATE_CONNECT:
      ...
      if (status == SIM7600_OK)
      {
        Serial.println("// ***** TEST GSM Connected OK");
        // ***** TEST GSM Connected OK
      ...
      {
        is_error = true;
        Serial.println("// ***** TEST GSM Connected Failed");
        // ***** TEST GSM Connected Failed
    ...
    case MODEM_STATE_CONNECTED:
      ...
      if (error)
      {        
        // ***** TEST GSM PPP Interface Establilished Connected FAIL
        Serial.println("// ***** TEST GSM PPP Interface Establilished Connected FAIL");
      ...
      {
        // ***** TEST GSM PPP Interface Establilished Connected OK
        Serial.println("// ***** TEST GSM PPP Interface Establilished Connected OK");
    ...
    case MODEM_STATE_DISCONNECT:
      ...
      if(param.system_status->connection.is_ppp_estabilished) {
        ...
        if (error)
        {
          // ***** TEST GSM PPP Interface Closing Connected FAIL
          Serial.println("// ***** TEST GSM PPP Interface Closing Connected FAIL");
          is_error = true;
          TRACE_ERROR_F(F("%s Failed to close PPP connection... [ %s ]\r\n"), Thread::GetName().c_str(), ERROR_STRING);
        } else {
          // ***** TEST GSM PPP Interface Closing Connected OK
          Serial.println("// ***** TEST GSM PPP Interface Closing Connected OK");
          param.systemStatusLock->Take();
      ...
      if (status == SIM7600_OK)
      {
        Serial.println("// ***** TEST GSM AT Closing Connection OK");
        // ***** TEST GSM AT Closing Connection OK
        is_error = false;
        state = MODEM_STATE_SWITCH_OFF;
      ...
      {
        Serial.println("// ***** TEST GSM AT Closing Connection FAIL");
        // ***** TEST GSM AT Closing Connection FAIL
        is_error = false;
        state = MODEM_STATE_SWITCH_OFF;
        TRACE_VERBOSE_F(F("MODEM_STATE_DISCONNECT -> MODEM_STATE_SWITCH_OFF\r\n"));
      }
    ...
    case MODEM_STATE_SWITCH_OFF:
      ...
      if (status == SIM7600_OK)
      {
        Serial.println("// ***** TEST GSM TurnOFF Power OK");
        // ***** TEST GSM TurnOFF Power OK
        is_error = false;
      ...
      {
        Serial.println("// ***** TEST GSM TurnOFF Power Failed");
        // ***** TEST GSM TurnOFF Power Failed
        is_error = false;
        state = MODEM_STATE_END;
        TRACE_VERBOSE_F(F("MODEM_STATE_SWITCH_OFF -> MODEM_STATE_END\r\n"));
      }
