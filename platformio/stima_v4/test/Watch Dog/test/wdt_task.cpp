...
void test_check_stalled_process(void);
void test_wdt_refresh(void);

// ************************************************************************
// ******************* TEST WDT FUNCTION IMPLEMENTATIONS ******************
// ************************************************************************

/**
 * @brief TEST: Check stalled process
 *
 */
void test_check_stalled_process() {
    TEST_ASSERT_TRUE(true);
}

/**
 * @brief TEST: Check Watchdog refresh
 *
 */
void test_wdt_refresh() {
    TEST_ASSERT_EQUAL(true, resetWdt);
}
            ...
            if (param.system_status->tasks[id].stack != 0xFFFFu) {
                switch (id) {
                    case SUPERVISOR_TASK_ID:
                        strcpy(strTask, "Supervisor  ");
                        break;
                    case MMC_TASK_ID:
                        strcpy(strTask, "MMC Card    ");
                        break;
                    case LCD_TASK_ID:
                        strcpy(strTask, "Display LCD ");
                        break;
                    case CAN_TASK_ID:
                        strcpy(strTask, "Can Bus     ");
                        break;
                    case MODEM_TASK_ID:
                        strcpy(strTask, "Modem 2G/4G ");
                        break;
                    case NTP_TASK_ID:
                        strcpy(strTask, "Net-NTP     ");
                        break;
                    case HTTP_TASK_ID:
                        strcpy(strTask, "Net-HTTP(S) ");
                        break;
                    case MQTT_TASK_ID:
                        strcpy(strTask, "NET MQTT(S) ");
                        break;
                    case USBSERIAL_TASK_ID:
                        strcpy(strTask, "USB SERIAL  ");
                        break;
                    case WDT_TASK_ID:
                        strcpy(strTask, "WDT Info    ");
                        break;
                }
                TRACE_INFO_F(F("%s%s : %d\r\n"), strTask,
                             (param.system_status->tasks[id].state == task_flag::suspended) ? SUSPEND_STRING : ((param.system_status->tasks[id].watch_dog == wdt_flag::clear) ? SPACE_STRING : FLAG_STRING),
                             param.system_status->tasks[id].stack);
            }
        }
#endif
        RUN_TEST(test_wdt_refresh);
