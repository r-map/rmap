...
void SdTask::Run()
{
  ...
  Serial.println("// ***** TEST SD CARD STARTING TEST OK");
  ...
  while (true)
  {
    ...
    case SD_STATE_CHECK_SD:
      ...
      // ***** TEST SD CARD INITIALIZE OK
      Serial.println("// ***** TEST SD CARD INITIALIZE OK");
      ...
      starting_test = true;
      // ***** TEST CREATE FILE FOR TEST OK
      Serial.println("// ***** TEST CREATE FILE FOR TEST OK");
      ...
      while(!param.dataRmapGetRequestQueue->IsEmpty()) {
        ...
        if(param.dataRmapGetRequestQueue->Dequeue(&rmap_get_request)) {
          ...
          if(rmap_get_request.command.do_synch_ptr) {
            ...
            Serial.println("// ***** TEST REQUEST SET POINTER DATA FROM QUEUE OK");
            ...
            if(SD.exists(rmap_file_name_new)) {
              ...
              while(true) {
                ...
                // Exist?
                if(SD.exists(rmap_file_name_new)) {
                  Serial.println("// ***** TEST FOUND FIRST FILE ABOUT POINTER REQUESTED (DATA POINTER NOT EXISTING) OK");
