...
//****************************************************************************************************
void test_request_data_sent(void);
...
void test_response() {
    ...
    TEST_ASSERT_EQUAL(rmap_service_setmode_1_0_test_acq, retData->stato);
}
...
#define MILLIS_EVENT 10
    ...
    CanardMicrosecond test_cmd_response_timeout = state.started_at + MEGA * 10;
    ...
    do {
        ...
        if (state.slave[queueId].is_online && test_state == INIT) {
            ...
            if (!state.slave[queueId].rmap_service.is_pending) {
                ...
                parametri.comando = rmap_service_setmode_1_0_test_acq;
                ...
                RUN_TEST(test_request_data_sent);
                test_state = REQUEST_DATA_SENT;
            }
        ...
        if (test_state == REQUEST_DATA_SENT) {
            if (state.slave[queueId].rmap_service.is_timeout) {
                ...
                bIsPendingData = state.slave[queueId].rmap_service.is_pending;
