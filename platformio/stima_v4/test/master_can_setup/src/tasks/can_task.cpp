...
CanTask::CanTask(const char *taskName, uint16_t stackSize, uint8_t priority, CanParam_t CanParam) : Thread(taskName, stackSize, priority), param(CanParam)
{
    ...
    if (!result) {
        ...
        if (!result) {
            // ***** TEST INIY bxCANComputeTimings on Chip FAILED
            Serial.println("// ***** TEST INIY bxCANComputeTimings on Chip FAILED");
            return;
        }
    }
    // ***** TEST INIY bxCANComputeTimings on Chip OK
    Serial.println("// ***** TEST INIY bxCANComputeTimings on Chip OK");
    ...
    if (!result) {
        // ***** TEST INIT bxCANConfigure on Chip FAILED
        Serial.println("// ***** TEST INIT bxCANConfigure on Chip FAILED");
        return;
    }

    // ***** TEST INIT bxCANConfigure on Chip OK
    Serial.println("// ***** TEST INIT bxCANConfigure on Chip OK");

    // *******************     CANARD SETUP TIMINGS AND SPEED COMPLETE   *******************

    // Check error starting CAN
    if (HAL_CAN_Start(&hcan1) != HAL_OK) {
        // ***** TEST INIT HAL CAN STM FAIL
        Serial.println("// ***** TEST INIT HAL CAN STM FAIL");
        return;
    }
    // ***** TEST INIT HAL CAN STM OK
    Serial.println("// ***** TEST INIT HAL CAN STM OK");

    // Enable Interrupt RX Standard CallBack -> CAN1_RX0_IRQHandler
    if (HAL_CAN_ActivateNotification(&hcan1, CAN_IT_RX_FIFO0_MSG_PENDING) != HAL_OK) {
        // ***** TEST INIT HAL CAN INTERRUPT FAIL
        Serial.println("// ***** TEST INIT HAL CAN INTERRUPT FAIL");
        return;
    }
    ...
    while (true) {
        ...
        switch (state) {
            ...
            case CAN_STATE_SETUP:
                ...
                if(is_init_subs_ok) {
                    // ***** TEST SUBSCRIPTION UAVCAN OK
                    Serial.println("// ***** TEST SUBSCRIPTION UAVCAN OK");
                } else {
                    // ***** TEST SUBSCRIPTION UAVCAN FAIL
                    Serial.println("// ***** TEST SUBSCRIPTION UAVCAN FAIL");
                }
