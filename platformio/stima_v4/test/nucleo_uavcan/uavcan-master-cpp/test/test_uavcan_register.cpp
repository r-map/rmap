...
//****************************************************************************************************
void test_register_sent(void);
void test_written_element_count_is_correct(void);
void test_reg_value_received_is_correct(void);
//****************************************************************************************************
...
enum test_status {
    INIT,
    REGISTER_SENT,
    RESPONSE_RECEIVED_MODE_WRITE,
    RESPONSE_RECEIVED_MODE_READ,
};
//****************************************************************************************************
...
void message_slave_offline() {
    TEST_ASSERT_TRUE_MESSAGE(false, "Slave node is offline");
}
...
void test_written_element_count_is_correct() {
    TEST_ASSERT_EQUAL(1, state.slave[queueId].register_access.response.unstructured.value.count);
}
...
void test_reg_value_received_is_correct() {
    TEST_ASSERT_EQUAL(value_to_write, state.slave[queueId].register_access.response.natural16.value.elements[0]);
}
...
#define MILLIS_EVENT 10
    ...
    CanardMicrosecond test_cmd_response_timeout = state.started_at + MEGA * 20;
    ...
    do {
        ...
        if (test_state == REGISTER_SENT) {
            if (state.slave[queueId].register_access.is_timeout) {
                ...
                bIsPendingRegister = state.slave[queueId].register_access.is_pending;
                ...
                if (bLastPendingRegister != bIsPendingRegister) {
                    ...
                    if (!bLastPendingRegister) {
                        if (bIsWriteRegister) {
                            RUN_TEST(test_written_element_count_is_correct);
                            test_state = RESPONSE_RECEIVED_MODE_WRITE;
                        } else {
                            RUN_TEST(test_reg_value_received_is_correct);
                            test_state = RESPONSE_RECEIVED_MODE_READ;
                        }
...
// Da utilizzare con CPU poco performanti in RX o con controllo Polling gestito Canard
    ...
    } while ((!state.flag.g_restart_required) && (monotonic_time <= test_cmd_response_timeout) && (test_state != RESPONSE_RECEIVED_MODE_READ));
