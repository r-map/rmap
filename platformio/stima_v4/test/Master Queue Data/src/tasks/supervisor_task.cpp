      ...
      if(starting_test) {
        ...
        if(rmap_get_response.result.done_synch) {
          ...
          if(rmap_get_response.rmap_data.date_time != 0) {
            Serial.println("// ***** TEST GET RESPONSE PTR WITH REAL EXISTING DATE OK");
          } else {
            Serial.println("// ***** TEST GET RESPONSE PTR WITH REAL EXISTING DATE FAIL");
          }
          ...
          if(rmap_get_response.rmap_data.date_time == rmap_date_time_ptr) {
            Serial.println("// ***** TEST SETTING PTR WITH REAL EXISTING REQUEST DATE OK");
          } else {
            Serial.println("// ***** TEST SETTING PTR WITH REAL EXISTING REQUEST DATE FAILED");
          }
        } else {
          Serial.println("// ***** TEST GET RESPONSE PTR WITH REAL EXISTING DATE FAIL");
        }
            ...
            if(!rmap_data_error) {
              ...
              if(rmap_get_response.result.done_get_data) {
                if(rmap_get_response.rmap_data.date_time == DATE_TIME_PTR_TEST) {
                  Serial.println("// ***** TEST SETTING PTR WITH DATE BEFORE EXISTING DATE OK");
                } else {
                  Serial.println("// ***** TEST SETTING PTR WITH DATE BEFORE EXISTING DATE FAILED");
                }
              }
            } else {
              Serial.println("// ***** TEST SETTING PTR WITH DATE BEFORE EXISTING DATE FAILED");
        ...
        char outString[256];
        uint32_t countData = 1; // Tot rec. read ( Starting with 1 because first data is getted UP to check TEST UP)
        ...
        while((!rmap_data_error)&&(!rmap_eof)) {
          ...
          if(!rmap_data_error) {
            ...
            if(bExecRepetedTest == false) {
              Serial.println("// ***** TEST GETTING NEXT QUEUE DATA OK");
            ...
            Serial.println("// ***** TEST GETTING NEXT QUEUE DATA ERROR");
        ...
        if((rmap_eof)&&(!rmap_data_error)) {
          Serial.println("// ***** TEST GETTING ALL QUEUE DATA OK");
        } else {
          Serial.println("// ***** TEST GETTING ALL QUEUE DATA FAILED");
        }
        if(countData == data_field_create) {
          Serial.println("// ***** TEST NUM DATA WRITE == NUM DATA READ OK");
        } else {
          Serial.println("// ***** TEST NUM DATA WRITE != NUM DATA READ FAILED");
        }
