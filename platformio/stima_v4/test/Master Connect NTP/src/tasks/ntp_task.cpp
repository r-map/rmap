...
void NtpTask::Run() {
  ...
  while (true)
  {
    ...
    case NTP_STATE_WAIT_NET_EVENT:
      ...
      if (param.connectionRequestQueue->Peek(&connection_request, portMAX_DELAY))
      {
        ...
        if (connection_request.do_ntp_sync)
        {
          ...
          Serial.println("// ***** TEST REQUEST QUEUE DO NTP INCOMING START");
    ...
    case NTP_STATE_DO_NTP_SYNC:
      ...
      if(error) {
        // ***** TEST INIT NTP CYCLONE FAILED
        Serial.println("// ***** TEST INIT NTP CYCLONE FAILED");
      } else {
        // ***** TEST INIT NTP CYCLONE OK
        Serial.println("// ***** TEST INIT NTP CYCLONE OK");
      }
      ...
      if (error)
      {
        ...
        // ***** TEST REQUEST RESOLVE NAME SERVER NTP FAILED
        Serial.println("// ***** TEST REQUEST RESOLVE NAME SERVER NTP FAILED");
        ...
        // ***** TEST REQUEST RESOLVE NAME SERVER NTP OK
        Serial.println("// ***** TEST REQUEST RESOLVE NAME SERVER NTP OK");
      }
      ...
      if (error)
      {
        ...
        TRACE_VERBOSE_F(F("NTP_STATE_DO_NTP_SYNC -> NTP_STATE_END\r\n"));
        // ***** TEST REQUEST RESOLVE NAME SERVER NTP OK
        Serial.println("// ***** TEST REQUEST RESOLVE NAME SERVER NTP TIME OUT FAIL");
        break;
      }
      ...
      if (!error)
      {
        // ***** TEST INIT NTP RETRIEVE TIME STAMP OK
        Serial.println("// ***** TEST INIT NTP RETRIEVE TIME STAMP OK");
        ...
        // TEST RELOAD EPOCH ON RTC HARDWARE
        if((test_epoch == unixTime)||(test_epoch == unixTime + 1)) {
          // ***** TEST RTC SET DATE TIME OK
          Serial.println("// ***** TEST RTC SET DATE TIME OK");
        } else {
          // ***** TEST RTC SET DATE TIME FAIL
          Serial.println("// ***** TEST RTC SET DATE TIME FAIL");
        }
      ...
      {
        // ***** TEST INIT NTP RETRIEVE TIME STAMP FAIL
        Serial.println("// ***** TEST INIT NTP RETRIEVE TIME STAMP FAIL");
