...
char test_low_power_enter = 0;
char test_low_power_exit = 0;
char test_os_tick_private = 0;
char vPortSuppressTicksAndSleep_exec = 0;
char test_low_power_enter_exec = 0;
char test_low_power_exit_exec = 0;
char test_os_tick_private_exec = 0;
char vPortSuppressTicksAndSleep_exec_exec = 0;

// ************************************************************************
// ***************** TEST LOWPOWER FUNCTION DECLARATIONS ******************
// ************************************************************************

void test_enter_vportsuppressticksandsleep(void);
void test_entering_low_power(void);
void test_exit_from_low_power(void);
void test_os_ticks_from_lptim1(void);

// ************************************************************************
// ***************** TEST LOWPOWER FUNCTION IMPLEMENTATIONS ***************
// ************************************************************************

/**
 * @brief TEST: Entering to function vPortSuppressTicksAndSleep
 *
 */
void test_enter_vportsuppressticksandsleep() {
    TEST_ASSERT_TRUE(true);
}

/**
 * @brief TEST: Entering low power sleep
 *
 */
void test_entering_low_power() {
    TEST_ASSERT_TRUE(true);
}

/**
 * @brief TEST: Exiting from lower power sleep
 *
 */
void test_exit_from_low_power() {
    TEST_ASSERT_TRUE(true);
}

/**
 * @brief TEST: OS ticks from timer LPTIM1
 *
 */
void test_os_ticks_from_lptim1() {
    TEST_ASSERT_TRUE(true);
}
...
void setup() {
    ...
    thSensorParam.elaborateDataQueue = elaborateDataQueue;
#endif
...
void loop() {
    ...
    // Test can be checked on LL_Low_Power enter wfy on STM32 LL4 Library...
    LowPower.idleHook();
    if (test_low_power_enter && !test_low_power_enter_exec) {
        RUN_TEST(test_entering_low_power);
        test_low_power_enter_exec = 1;
    }
    if (test_low_power_exit && !test_low_power_exit_exec) {
        RUN_TEST(test_exit_from_low_power);
        test_low_power_exit_exec = 1;
    }
    if (test_os_tick_private && !test_os_tick_private_exec) {
        RUN_TEST(test_os_ticks_from_lptim1);
        test_os_tick_private_exec = 1;
    }
    if (vPortSuppressTicksAndSleep_exec && !vPortSuppressTicksAndSleep_exec_exec) {
        RUN_TEST(test_enter_vportsuppressticksandsleep);
        vPortSuppressTicksAndSleep_exec_exec = 1;
    }
    if (test_low_power_enter && test_low_power_exit && test_os_tick_private && vPortSuppressTicksAndSleep_exec) {
        UNITY_END();
