...
void SupervisorTask::loadConfiguration()
{
  ...
  if(register_config_valid) {
    ...
    uavcan_register_Value_1_0_select_real32_(&val);
    val.real32.value.count       = MAX_ADC_CHANELS;
    // Loading Default ( Analog Min is SOIL_VWC_VOLTAGE_MIN )
    for(idx=0; idx<MAX_ADC_CHANELS; idx++)
      val.real32.value.elements[idx] = SOIL_VWC_VOLTAGE_MIN;
    param.registerAccessLock->Take();
  ...
  if(register_config_valid) {
    ...
    uavcan_register_Value_1_0_select_real32_(&val);
    val.real32.value.count       = MAX_ADC_CHANELS;
    // Loading Default ( Analog Max is SOIL_VWC_VOLTAGE_MAX )
    for(idx=0; idx<MAX_ADC_CHANELS; idx++)
      val.real32.value.elements[idx] = SOIL_VWC_VOLTAGE_MAX;
    param.registerAccessLock->Take();
...
void SupervisorTask::saveConfiguration(bool is_default)
{
  ...
  if(is_default) {
    ...
    for(idx=0; idx<MAX_ADC_CHANELS; idx++) {
      ...
      param.configuration->sensors[idx].analog_min = SOIL_VWC_VOLTAGE_MIN;
      param.configuration->sensors[idx].analog_max = SOIL_VWC_VOLTAGE_MAX;
