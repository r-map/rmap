...
void CanTask::prepareSensorsDataValue(uint8_t const sensore, const report_t *report, rmap_module_VWC_1_0 *rmap_data) {
...
void CanTask::prepareSensorsDataValue(uint8_t const sensore, const report_t *report, rmap_service_module_VWC_Response_1_0 *rmap_data) {
    // Inserisco i dati reali
    switch (sensore) {
        case canardClass::Sensor_Type::vwc1:
            // Prepara i dati SMP (Sample)
...
void CanTask::publish_rmap_data(canardClass &clCanard, CanParam_t *param) {
    ...
    if ((!clCanard.is_canard_node_anonymous()) &&
        (clCanard.publisher_enabled.module_soil_vwc) &&
        (clCanard.port_id.publisher_module_soil_vwc <= CANARD_SUBJECT_ID_MAX)) {
        rmap_module_VWC_1_0 module_soil_vwc_msg = {0};
        ...
        clCanard.module_soil_vwc1.VWC.metadata.timerange.P2 = request_data.report_time_s;
        ...
        // Metadata
        module_soil_vwc_msg.VWC.metadata = clCanard.module_soil_vwc1.VWC.metadata;

        // Serialize and publish the message:
        uint8_t serialized[rmap_module_VWC_1_0_SERIALIZATION_BUFFER_SIZE_BYTES_] = {0};
        size_t serialized_size = sizeof(serialized);
        const int8_t err = rmap_module_VWC_1_0_serialize_(&module_soil_vwc_msg, &serialized[0], &serialized_size);
        LOCAL_ASSERT(err >= 0);
        if (err >= 0) {
            const CanardTransferMetadata meta = {
                .priority = CanardPrioritySlow,
                .transfer_kind = CanardTransferKindMessage,
                .port_id = clCanard.port_id.publisher_module_soil_vwc,
                .remote_node_id = CANARD_NODE_ID_UNSET,
                .transfer_id = (CanardTransferID)(clCanard.next_transfer_id.module_soil_vwc()),  // Increment!
            };
...
uavcan_node_ExecuteCommand_Response_1_1 CanTask::processRequestExecuteCommand(canardClass &clCanard, const uavcan_node_ExecuteCommand_Request_1_1* req,
    ...
    switch (req->command)
    {
        ...
        case canardClass::Command_Private::enable_publish_rmap:
        {
            ...
            clCanard.publisher_enabled.module_soil_vwc = true;
        ...
        case canardClass::Command_Private::disable_publish_rmap:
        {
            ...
            clCanard.publisher_enabled.module_soil_vwc = false;
...
rmap_service_module_VWC_Response_1_0 CanTask::processRequestGetModuleData(canardClass &clCanard, rmap_service_module_VWC_Request_1_0* req, CanParam_t *param) {
    rmap_service_module_VWC_Response_1_0 resp = {0};
    ...
    switch (req->parameter.command) {
        ...
        case rmap_service_setmode_1_0_get_last:     // saturated uint3 get_last = 2
          ...
          if((req->parameter.command == rmap_service_setmode_1_0_get_current)||
            ...
            // SET Dynamic metadata (Request data from master Only Data != Sample)
            clCanard.module_soil_vwc1.VWC.metadata.timerange.P2 = request_data.report_time_s;
            clCanard.module_soil_vwc2.VWC.metadata.timerange.P2 = request_data.report_time_s;
            clCanard.module_soil_vwc3.VWC.metadata.timerange.P2 = request_data.report_time_s;
          }

          // Preparo il ritorno dei flag event status del sensore (Prima di request/reset)
          resp.is_adc_unit_error = param->system_status->events.is_adc_unit_error;
          resp.is_adc_unit_overflow = param->system_status->events.is_adc_unit_overflow;
          ...
          prepareSensorsDataValue(canardClass::Sensor_Type::vwc1, &report_srv, &resp);
          prepareSensorsDataValue(canardClass::Sensor_Type::vwc2, &report_srv, &resp);
          prepareSensorsDataValue(canardClass::Sensor_Type::vwc3, &report_srv, &resp);
          break;
    ...
    resp.VWC1.metadata = clCanard.module_soil_vwc1.VWC.metadata;
    resp.VWC2.metadata = clCanard.module_soil_vwc2.VWC.metadata;
    resp.VWC3.metadata = clCanard.module_soil_vwc3.VWC.metadata;
...
void CanTask::processReceivedTransfer(canardClass &clCanard, const CanardRxTransfer* const transfer, void *param) {
        ...
        if (transfer->metadata.port_id == uavcan_node_Heartbeat_1_0_FIXED_PORT_ID_)
        {
            ...
            if (uavcan_node_Heartbeat_1_0_deserialize_(&msg, static_cast<uint8_t const*>(transfer->payload), &size) >= 0) {
                ...
                if(transfer->metadata.remote_node_id == clCanard.get_canard_master_id()) {
                    ...
                    TRACE_INFO_F(F("RX HeartBeat from master, master power mode SET: -> %u\r\n"), remoteVSC.powerMode);
    ...
    else if (transfer->metadata.transfer_kind == CanardTransferKindRequest)
    {
        ...
        if (transfer->metadata.port_id == clCanard.port_id.service_module_soil_vwc) {
            // Richiesta ai dati e metodi di sensor drive
            rmap_service_module_VWC_Request_1_0 req = {0};
            size_t size = transfer->payload_size;
            TRACE_INFO_F(F("<<-- Ricevuto richiesta dati da master\r\n"));
            // The request object is empty so we don't bother deserializing it. Just send the response.
            if (rmap_service_module_VWC_Request_1_0_deserialize_(&req, static_cast<uint8_t const*>(transfer->payload), &size) >= 0) {
                // I dati e metadati sono direttamente popolati in processRequestGetModuleData
                rmap_service_module_VWC_Response_1_0 module_VWC_resp = processRequestGetModuleData(clCanard, &req, (CanParam_t *) param);
                // Serialize and publish the message:
                uint8_t serialized[rmap_service_module_VWC_Response_1_0_SERIALIZATION_BUFFER_SIZE_BYTES_] = {0};
                size_t serialized_size = sizeof(serialized);
                const int8_t res = rmap_service_module_VWC_Response_1_0_serialize_(&module_VWC_resp, &serialized[0], &serialized_size);
                if (res >= 0) {
    ...
    while (true) {
        ...
        switch (state) {
            ...
            case CAN_STATE_INIT:
                ...
                clCanard.port_id.publisher_module_soil_vwc = (CanardPortID)SUBJECTID_PUBLISH_RMAP;
                #else
                clCanard.port_id.publisher_module_soil_vwc =
                    getModeAccessID(canardClass::Introspection_Port::PublisherSubjectID,
                        REGISTER_DATA_PUBLISH, rmap_module_VWC_1_0_FULL_NAME_AND_VERSION_);
                #endif
                #ifdef USE_PORT_SERVICE_RMAP_FIXED
                clCanard.port_id.service_module_soil_vwc = (CanardPortID)PORT_SERVICE_RMAP;
                #else
                clCanard.port_id.service_module_soil_vwc =
                    getModeAccessID(canardClass::Introspection_Port::ServicePortID,
                        REGISTER_DATA_SERVICE, rmap_service_module_VWC_1_0_FULL_NAME_AND_VERSION_);
                #endif
                ...
                if(clCanard.port_id.service_module_soil_vwc != UINT16_MAX)
                ...
                // Set Default difference measure
                val.natural16.value.elements[SENSOR_METADATA_VWC1] = SENSOR_METADATA_LEVEL_1_VWC1;
                val.natural16.value.elements[SENSOR_METADATA_VWC2] = SENSOR_METADATA_LEVEL_1_VWC2;
                val.natural16.value.elements[SENSOR_METADATA_VWC3] = SENSOR_METADATA_LEVEL_1_VWC3;
                localRegisterAccessLock->Take();
                ...
                clCanard.module_soil_vwc1.VWC.metadata.level.L1.value = val.natural16.value.elements[SENSOR_METADATA_VWC1];
                clCanard.module_soil_vwc2.VWC.metadata.level.L1.value = val.natural16.value.elements[SENSOR_METADATA_VWC2];
                clCanard.module_soil_vwc3.VWC.metadata.level.L1.value = val.natural16.value.elements[SENSOR_METADATA_VWC3];
                // *********************************** L2 *********************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_soil_vwc1.VWC.metadata.level.L2.value = val.natural16.value.elements[SENSOR_METADATA_VWC1];
                clCanard.module_soil_vwc2.VWC.metadata.level.L2.value = val.natural16.value.elements[SENSOR_METADATA_VWC2];
                clCanard.module_soil_vwc3.VWC.metadata.level.L2.value = val.natural16.value.elements[SENSOR_METADATA_VWC3];
                // ******************************* LevelType1 *****************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                for(uint8_t id=0; id<SENSOR_METADATA_COUNT; id++) {
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_soil_vwc1.VWC.metadata.level.LevelType1.value = val.natural16.value.elements[SENSOR_METADATA_VWC1];
                clCanard.module_soil_vwc2.VWC.metadata.level.LevelType1.value = val.natural16.value.elements[SENSOR_METADATA_VWC2];
                clCanard.module_soil_vwc3.VWC.metadata.level.LevelType1.value = val.natural16.value.elements[SENSOR_METADATA_VWC3];
                // ******************************* LevelType2 *****************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                for(uint8_t id=0; id<SENSOR_METADATA_COUNT; id++) {
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_soil_vwc1.VWC.metadata.level.LevelType2.value = val.natural16.value.elements[SENSOR_METADATA_VWC1];
                clCanard.module_soil_vwc2.VWC.metadata.level.LevelType2.value = val.natural16.value.elements[SENSOR_METADATA_VWC2];
                clCanard.module_soil_vwc3.VWC.metadata.level.LevelType2.value = val.natural16.value.elements[SENSOR_METADATA_VWC3];
                // *********************************** P1 *********************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                for(uint8_t id=0; id<SENSOR_METADATA_COUNT; id++) {
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_soil_vwc1.VWC.metadata.timerange.P1.value = val.natural16.value.elements[SENSOR_METADATA_VWC1];
                clCanard.module_soil_vwc2.VWC.metadata.timerange.P1.value = val.natural16.value.elements[SENSOR_METADATA_VWC2];
                clCanard.module_soil_vwc3.VWC.metadata.timerange.P1.value = val.natural16.value.elements[SENSOR_METADATA_VWC3];
                // *********************************** P2 *********************************************
                // P2 Non memorizzato sul modulo, parametro dipendente dall'acquisizione locale
                clCanard.module_soil_vwc1.VWC.metadata.timerange.P2 = 0;
                clCanard.module_soil_vwc2.VWC.metadata.timerange.P2 = 0;
                clCanard.module_soil_vwc3.VWC.metadata.timerange.P2 = 0;
                // *********************************** P2 *********************************************
                ...
                val.natural8.value.elements[SENSOR_METADATA_VWC1] = SENSOR_METADATA_LEVEL_P_IND_VWC1;
                val.natural8.value.elements[SENSOR_METADATA_VWC2] = SENSOR_METADATA_LEVEL_P_IND_VWC2;
                val.natural8.value.elements[SENSOR_METADATA_VWC3] = SENSOR_METADATA_LEVEL_P_IND_VWC3;
                localRegisterAccessLock->Take();
                ...
                clCanard.module_soil_vwc1.VWC.metadata.timerange.Pindicator.value = val.natural8.value.elements[SENSOR_METADATA_VWC1];
                clCanard.module_soil_vwc2.VWC.metadata.timerange.Pindicator.value = val.natural8.value.elements[SENSOR_METADATA_VWC2];
                clCanard.module_soil_vwc3.VWC.metadata.timerange.Pindicator.value = val.natural8.value.elements[SENSOR_METADATA_VWC3];
            ...
            case CAN_STATE_SETUP:
                ...
                if (!clCanard.rxSubscribe(CanardTransferKindRequest,
                                        clCanard.port_id.service_module_soil_vwc,
                                        rmap_service_module_VWC_Request_1_0_EXTENT_BYTES_,
