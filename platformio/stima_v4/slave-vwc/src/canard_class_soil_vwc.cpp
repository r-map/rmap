/**
  ******************************************************************************
  * @file    canard_class_soil_vwc.cpp
...
canardClass::canardClass() {
    ...
    port_id.publisher_module_soil_vwc = UINT16_MAX;
    port_id.service_module_soil_vwc = UINT16_MAX;

    publisher_enabled.module_soil_vwc = DEFAULT_PUBLISH_MODULE_DATA;
...
bool canardClass::slave_servicelist_send_message(void)
{
        ...
        (_canard.node_id <= CANARD_NODE_ID_MAX))
    {
        ...
        // Indicate which subjects we publish to. Don't forget to keep this updated if you add new publications!
        {
            ...
            if ((port_id.publisher_module_soil_vwc <= CANARD_SUBJECT_ID_MAX)&&
                (publisher_enabled.module_soil_vwc))
            {
                m.publishers.sparse_list.elements[(*cnt)++].value = port_id.publisher_module_soil_vwc;
            }
        }
