...
//Global variable
static Stm32l5xxRsaArgs rsaArgs;
static Stm32l5xxEccArgs eccArgs;
...
error_t pkaModExp(Mpi *r, const Mpi *a, const Mpi *e, const Mpi *p)
{
   ...
   if(modLen <= STM32L5XX_MAX_ROS && expLen <= STM32L5XX_MAX_ROS)
   {
...
error_t pkaRsaCrtExp(const RsaPrivateKey *key, const Mpi *c, Mpi *m)
{
   ...
   //Check the length of the operands
   if(nLen <= STM32L5XX_MAX_ROS && pLen <= (nLen / 2) && qLen <= (nLen / 2) &&
   ...
   const EcPoint *s)
{
   ...
   //Check the length of the operands
   if(modLen <= STM32L5XX_MAX_EOS && scalarLen <= STM32L5XX_MAX_EOS)
   {
      //Acquire exclusive access to the PKA module
      osAcquireMutex(&stm32l5xxCryptoMutex);
   ...
   const uint8_t *digest, size_t digestLen, EcdsaSignature *signature)
{
   ...
   //Check the length of the operands
   if(modLen > STM32L5XX_MAX_EOS || orderLen > STM32L5XX_MAX_EOS)
