...
/**
 * @brief CHECK IF THE COMMAND SENT TO SLAVE
 *
 */
void test_command_sent() {
    TEST_ASSERT_TRUE(serviceSendCommand(&state, monotonic_time, queueId,
                                        CMD_TEST, NULL, 0));
}

/**
 * @brief CHECK IF THE RESPONSE FROM SLAVE IS CORRECT
 *
 */
