...
void SupervisorTask::printConfiguration()
{
  ...
  {
    //TODO:_TH_RAIN_ SENSOR_DRIVER_I2C PRINT -> "PG."
    TRACE_INFO_F(F("--> %u: %s-%s 0x%02X [ %s ]\r\n"), i + 1, "PG.", param.configuration->sensors[i].type, param.configuration->sensors[i].i2c_address, param.configuration->sensors[i].is_redundant ? REDUNDANT_STRING : MAIN_STRING);
  }
