...
#define TRACE_LEVEL     LEVEL_SENSOR_TASK_TRACE_LEVEL
#define LOCAL_TASK_ID   SENSOR_TASK_ID

#include "tasks/level_sensor_task.h"
...
/// @param levelSensorParam local parameters for the task
LevelSensorTask::LevelSensorTask(const char *taskName, uint16_t stackSize, uint8_t priority, LevelSensorParam_t levelSensorParam) : Thread(taskName, stackSize, priority), param(levelSensorParam)
{
...
void LevelSensorTask::TaskMonitorStack()
{
...
void LevelSensorTask::TaskWatchDog(uint32_t millis_standby)
{
...
void LevelSensorTask::TaskState(uint8_t state_position, uint8_t state_subposition, task_flag state_operation)
{
  // Local TaskWatchDog update
  param.systemStatusLock->Take();
...
void LevelSensorTask::Run() {
  ...
  #if (LEVEL_TASK_LOW_POWER_ENABLED)
  powerOff();
  #else
  ...
  while (true)
  {
    ...
    case SENSOR_STATE_WAIT_CFG:
      ...
      TaskWatchDog(LEVEL_TASK_WAIT_DELAY_MS);
      Delay(Ticks::MsToTicks(LEVEL_TASK_WAIT_DELAY_MS));
      break;

    case SENSOR_STATE_INIT:
      TRACE_INFO_F(F("Initializing sensors...\r\n"));
      #if (LEVEL_TASK_LOW_POWER_ENABLED) // Auto power chanel with switching mode and power time waiting...
      powerOn(adc_channel);
      #endif
...
void LevelSensorTask::powerOn(uint8_t chanel_out)
{
  if (!is_power_on) {
    ...
    #endif
    // WDT
    TaskWatchDog(LEVEL_TASK_POWER_ON_WAIT_DELAY_MS);
    Delay(Ticks::MsToTicks(LEVEL_TASK_POWER_ON_WAIT_DELAY_MS));
    is_power_on = true;
  }
}

/// @brief Power OFF Method (rest power mode)
void LevelSensorTask::powerOff()
{
  digitalWrite(PIN_EN_5VA, LOW);  // Disable Analog Comparator (xIAN)
  #if(LEVEL_TASK_SWITCH_POWER_ALIM_SENS)
...
uint8_t LevelSensorTask::addADCData(uint8_t chanel_out)
{
...
void LevelSensorTask::resetADCData(uint8_t chanel_out)
{
...
float LevelSensorTask::getADCData(uint8_t chanel_out, uint8_t *quality_data)
{
  *quality_data = 0;
...
int32_t LevelSensorTask::getVrefTemp(void)
{
...
float LevelSensorTask::getAdcCalibratedValue(float adc_value, float offset, float gain)
{
...
float LevelSensorTask::getAdcAnalogValue(float adc_value, Adc_Mode adc_type)
{
  float min, max;
  float value = (float)UINT16_MAX;
...
float LevelSensorTask::getLevel(float adc_value, float adc_current_min, float adc_current_max, bool *adc_overflow)
{
  ...
  *adc_overflow = false;
  ...
  if (value < LEVEL_ERROR_CURRENT_MIN || value > LEVEL_ERROR_CURRENT_MAX)
  {
    *adc_overflow = true;
    value = UINT16_MAX;
  }
