...
void ElaborateDataTask::Run() {
  ...
  bufferReset<sample_t, uint16_t, rmapdata_t>(&level_samples, SAMPLES_COUNT_MAX);
  ...
  for(uint16_t iInit=0; iInit<900; iInit++) {
    ...
    TRACE_VERBOSE_F(F("Level: %d\r\n"), edata.value);
    addValue<maintenance_t, uint16_t, bool>(&maintenance_samples, SAMPLES_COUNT_MAX, false);
    addValue<sample_t, uint16_t, rmapdata_t>(&level_samples, SAMPLES_COUNT_MAX, edata.value);
  }
  #endif
...
/// @param level real value readed from sensor
/// @return value uint_8 percent data quality value
uint8_t ElaborateDataTask::checkLevel(rmapdata_t level) {
...
void ElaborateDataTask::make_report (bool is_init, uint16_t report_time_s, uint8_t observation_time_s) {
  ...
  // Elaboration level
  rmapdata_t level_s = 0;
  float avg_level_s = 0;
  float avg_level_o = 0;
  float min_level_o = FLT_MAX;
  float max_level_o = FLT_MIN;
  float avg_level_quality_s = 0;
  float avg_level_quality_o = 0;
  #if TRACE_LEVEL >= TRACE_LEVEL_DEBUG
  uint16_t valid_count_level_t = 0;
  #endif
  uint16_t valid_count_level_s = 0;
  uint16_t total_count_level_s = 0;
  uint16_t valid_count_level_o = 0;
  ...
  else
  {
    ...
    TRACE_DEBUG_F(F("-> %d available level samples count\r\n"), level_samples.count);
  }
  ...
  bufferPtrResetBack<sample_t, uint16_t>(&level_samples, SAMPLES_COUNT_MAX);

  // align all sensor's data to last common acquired sample
  uint16_t samples_count = level_samples.count;

  // it's a simple istant or report request?
  if (report_time_s == 0)
  {
    // Make last data value to Get Istant show value
    report.avg = bufferReadBack<sample_t, uint16_t, rmapdata_t>(&level_samples, SAMPLES_COUNT_MAX);
  }
  else
  {
    for (uint16_t i = 0; i < samples_count; i++)
    {
      ...
      level_s = bufferReadBack<sample_t, uint16_t, rmapdata_t>(&level_samples, SAMPLES_COUNT_MAX);
      ...
      #endif
      total_count_level_s++;
      avg_level_quality_s += (float)(((float)checkLevel(level_s) - avg_level_quality_s) / total_count_level_s);
      if ((ISVALID_RMAPDATA(level_s)) && !measures_maintenance)
      {
        valid_count_level_s++;
        #if TRACE_LEVEL >= TRACE_LEVEL_DEBUG
        valid_count_level_t++;
        #endif
        avg_level_s += (float)(((float)level_s - avg_level_s) / valid_count_level_s);
      }
      ...
      if(is_observation) {
        ...
        // level, sufficient number of valid samples?
        valid_data_calc_perc = (float)(valid_count_level_s) / (float)(total_count_level_s) * 100.0;
        if (valid_data_calc_perc >= SAMPLE_ERROR_PERCENTAGE_MIN)
        {
          valid_count_level_o++;
          avg_level_o += (avg_level_s - avg_level_o) / valid_count_level_o;
          avg_level_quality_o += (avg_level_quality_s - avg_level_quality_o) / valid_count_level_o;
          // Elaboration MIN and MAX for observation
          if(avg_level_s < min_level_o) min_level_o = avg_level_s;
          if(avg_level_s > max_level_o) max_level_o = avg_level_s;
        }
        // Reset Buffer sample for calculate next observation
        avg_level_quality_s = 0;
        avg_level_s = 0;
        valid_count_level_s = 0;
        total_count_level_s = 0;
      }
    }
    ...
    valid_data_calc_perc = (float)(valid_count_level_o) / (float)(report_observations_count) * 100.0;
    TRACE_DEBUG_F(F("-> %d level sample error (%d%%)\r\n"), (n_sample - valid_count_level_t), (uint8_t)(((float)n_sample - (float)valid_count_level_t)/(float)n_sample * 100.0));
    TRACE_DEBUG_F(F("-> %d level observation avaiable (%d%%)\r\n"), valid_count_level_o, (uint8_t)valid_data_calc_perc);
    if (valid_data_calc_perc >= OBSERVATION_ERROR_PERCENTAGE_MIN)
    {
      // report.temperature.ist (already assigned)
      report.avg = (rmapdata_t)avg_level_o;
      // report.min = (rmapdata_t)min_level_o;
      // report.max = (rmapdata_t)max_level_o;
      report.quality = (rmapdata_t)avg_level_quality_o;
    }

    // Trace report final
    TRACE_INFO_F(F("--> level report\t%d\t%d\r\n"), (rmapdata_t)report.avg, (rmapdata_t)report.quality);
  }
}
