...
// TODO: VERIFICA CAN_REDUNDANCY_FACTOR SU MULTICOMANDO/MULTISCHEDA..
...
// INIT REGISTER INTERNI
// #define NODE_FIXED_ID 10
...
// TODO: Microsecond register STM32...
...
void CAN_HW_Init(void) {
    ...
    // CAN cell initialization - BAUD RATE = (80M / (SJW + TS1 + TS2)) / Prescaler
    //                         > BAUD RATE = (80M / (5 * (1 + 13 + 2))) -> = 1 Mbit/s
    ...
    if (HAL_CAN_Init(&CAN_Handle) != HAL_OK) {
        Serial.println(F("Error initialization CAN"));
        Error_Handler();
    }
    ...
    if (HAL_CAN_ConfigFilter(&CAN_Handle, &CAN_FilterInitStruct) != HAL_OK) {
        Serial.println(F("Error initialization CAN filter"));
        Error_Handler();
    }

    // SetUp Speed to 250 Khz
    ...
    // TODO: SetUp Interrupt + HandleInterrupt + CallBack
    ...
    if (HAL_CAN_Start(&CAN_Handle) != HAL_OK) {
        Serial.println(F("Error start CAN"));
        Error_Handler();
    }

    // Print status
    Serial.println(F("CAN1 start"));
}
...
void setup(void) {
    ...
    // ***********************************************
    //      STARTUP SD-CARD FROM REGISTER MODULE
    // ***********************************************
    // Setup della Lib SD e SPIClass
    ...
    //                STARTUP CANBUS 
    // ***********************************************
    Serial.print(F("Initializing... PCLK1 Clock Freq: "));
    Serial.println(HAL_RCC_GetPCLK1Freq());
    CAN_HW_Init();
 
    // ***********************************************
    //        STARTUP LIBCANARD - OPENCYPAL 
    // ***********************************************

    // TODO: Cypal Insert Setup, InitRegister ecc...
    // Inserire quà il setup / Load parametrizzato
    // Define x Fixed Test Using...

    // N.B. BITRATE PUO CAMBIARE, settare l'opportuna VAR in registerWrite... e restart
    // e gestire SAVE/READ, oppure Fisso con INIT generico... fisso x MTU/BITRATE
}
...
void loop(void)
{
    ...
    val.natural16.value.elements[0] = CAN_BIT_RATE;
    val.natural16.value.elements[1] = 0ul;          // Ignored for CANARD_MTU_CAN_CLASSIC
    ...
    uavcan_register_Value_1_0_select_natural16_(&val);
    val.natural16.value.count = 1;
    val.natural16.value.elements[0] = NODE_FIXED_ID; // Fixed NodeID, per Specification/libcanard.
    ...
    val.natural16.value.count = 1;
    val.natural16.value.elements[0] = NODE_FIXED_ID;  // Fixed NodeId Value for Define
    state.canard.node_id = (CanardNodeID) val.natural16.value.elements[0];
    #else
    uavcan_register_Value_1_0_select_natural16_(&val);
    val.natural16.value.count = 1;
    ...
    uavcan_register_Value_1_0_select_natural16_(&val);
    val.natural16.value.count       = 1;
    val.natural16.value.elements[0] = CANARD_NODE_ID_UNSET;
    ...
    do
    {
        ...
        for (uint8_t ifidx = 0; ifidx < CAN_REDUNDANCY_FACTOR; ifidx++)
        {
            ...
            if (bxCANPop(0, &frame.extended_can_id, &frame.payload_size, &frame.payload)) {
                ...
                {
                    // No other error can possibly occur at runtime.
                    assert(false);
                }
            }
        }
