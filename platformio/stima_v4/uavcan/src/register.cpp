...
// *********************************************************
// Preselzione del metodo di accesso ai registri OPENCYPHAL
// SD - SPI FLASH - E2PROM - ALTRI...
// *********************************************************
// #define USE_FS_SYSSTAT
// #define USE_LIB_SD
#define USE_LIB_SDFAT
// #define USE_LIB_FLASH_SPI
// #define USE_STIMA4_E2PROM
// *********************************************************
...
#ifdef USE_LIB_SD
// Classic ARDUINO SD 8.3 NANING (Small ROM/RAM Using)
#   include <SPI.h>
#   include <SD.h>
#endif

#ifdef USE_LIB_SDFAT
// Advanced ARDUINO SD LONG NANING CONVENTION
#   include <SPI.h>
#   include "SdFat.h"
#endif
...
// Define MAX space buffer Limit Uavcan STRING to 100 Bytes (unattainable limit)
#ifdef USE_REGISTER_MINIMAL_RAM
    #define REGISTER_SIZE_LIMIT 100U
    #define REGISTER_SIZE_LIMIT_EXTENT REGISTER_SIZE_LIMIT + 4U
#else
    #define REGISTER_SIZE_LIMIT 100U#USE_REGISTER_MINIMAL_RAM
    #define REGISTER_SIZE_LIMIT_EXTENT uavcan_register_Value_1_0_EXTENT_BYTES_
#endif

static const char registryDirName[] = "regs/";
static const char registryListName[] = "regs/list.txt";

// TODO: 1) TEST Funzioni, 2) Portabilità su Flash/E2Prom con stesse chimate
// SD_CARD FAT STANDARD ARDUINO 8.3 NAMING UTILITY
void getRegisterIndexName(byte registerIndex, char *strData);
char getRegisterFileName(byte registerIndex, char *strData);
void registerInit(void);
bool registerListFind(const char* const register_name, byte *registerNumber);
bool registerListAppend(const char* const register_name, byte registerIndex);
...
char getRegisterFileName(byte registerIndex, char *strData) {
    ...
    getRegisterFileName(registerIndex, strIndexName);
...
void registerInit(void)
{
    ...
    if(!SD.exists(registryListName)) {
        ...
        // UniqueId (UINT32 Standard) 8 BITS HWSpecific Module + 24 Bit Random (S.N.)
        byte uniqueNodeId[4];
        for(int i=0;i<3;i++)
            uniqueNodeId[i] = (rand() & 0xFF);        
        uniqueNodeId[3] = NODE_TYPE;
        ...
        registerPut(0, uniqueNodeId, sizeof(uniqueNodeId));
    }
}
...
bool registerListFind(const char* const register_name, byte *registerNumber)
{
    ...
    char register_read[REGISTER_SIZE_LIMIT];    
    char strIndexName[REGISTER_F_SIZE + 1];
...
void registerPut(byte registerIndex, byte *registerValue, byte registerLength)
{
    ...
    registryFileData = SD.open(registryFileReg, FILE_WRITE);
...
void registerRead(const char* const register_name, uavcan_register_Value_1_0* const inout_value)
{
    ...
    if (registerFound)
    {
        ...
        uint8_t serialized[REGISTER_SIZE_LIMIT_EXTENT] = {0};    
        size_t  sr_size = registerGet(registerIndex, &serialized[0], REGISTER_SIZE_LIMIT_EXTENT);
...
void registerWrite(const char* const register_name, const uavcan_register_Value_1_0* const value)
{
    ...
    uint8_t serialized[REGISTER_SIZE_LIMIT_EXTENT] = {0};
    size_t sr_size = REGISTER_SIZE_LIMIT_EXTENT;
...
#endif
//////////////////////////////////////////////////////////////////////////////////////////
//                          FILE SYTEM STANDARD ORIGINAL CYPAL
//////////////////////////////////////////////////////////////////////////////////////////
#ifdef USE_FS_SYSSTAT
...
// ************************************************************************************
//                          UAVCAN RegisterAssign Standard
// ************************************************************************************
