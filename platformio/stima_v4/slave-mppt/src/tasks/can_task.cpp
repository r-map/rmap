...
void CanTask::prepareSensorsDataValue(uint8_t const sensore, const report_t *report, rmap_service_module_Power_Response_1_0 *rmap_data) {
    // Inserisco i dati reali
    switch (sensore) {
        case canardClass::Sensor_Type::dep:
            // Prepara i dati SMP (Sample)
...
void CanTask::publish_rmap_data(canardClass &clCanard, CanParam_t *param) {
    ...
    if ((!clCanard.is_canard_node_anonymous()) &&
        (clCanard.publisher_enabled.module_mppt) &&
        (clCanard.port_id.publisher_module_mppt <= CANARD_SUBJECT_ID_MAX)) {
        ...
        const int8_t err = rmap_module_Power_1_0_serialize_(&module_mppt_msg, &serialized[0], &serialized_size);
        LOCAL_ASSERT(err >= 0);
        if (err >= 0) {
            const CanardTransferMetadata meta = {
                .priority = CanardPrioritySlow,
                .transfer_kind = CanardTransferKindMessage,
                .port_id = clCanard.port_id.publisher_module_mppt,
                .remote_node_id = CANARD_NODE_ID_UNSET,
                .transfer_id = (CanardTransferID)(clCanard.next_transfer_id.module_mppt()),  // Increment!
            };
...
uavcan_node_ExecuteCommand_Response_1_1 CanTask::processRequestExecuteCommand(canardClass &clCanard, const uavcan_node_ExecuteCommand_Request_1_1* req,
    ...
    switch (req->command)
    {
        ...
        case canardClass::Command_Private::enable_publish_rmap:
        {
            ...
            clCanard.publisher_enabled.module_mppt = true;
        ...
        case canardClass::Command_Private::disable_publish_rmap:
        {
            ...
            clCanard.publisher_enabled.module_mppt = false;
...
rmap_service_module_Power_Response_1_0 CanTask::processRequestGetModuleData(canardClass &clCanard, rmap_service_module_Power_Request_1_0* req, CanParam_t *param) {
    rmap_service_module_Power_Response_1_0 resp = {0};
...
void CanTask::processReceivedTransfer(canardClass &clCanard, const CanardRxTransfer* const transfer, void *param) {
    ...
    else if (transfer->metadata.transfer_kind == CanardTransferKindRequest)
    {
        ...
        if (transfer->metadata.port_id == clCanard.port_id.service_module_mppt) {
            // Richiesta ai dati e metodi di sensor drive
            rmap_service_module_Power_Request_1_0 req = {0};
            size_t size = transfer->payload_size;
            TRACE_INFO_F(F("<<-- Ricevuto richiesta dati da master\r\n"));
            // The request object is empty so we don't bother deserializing it. Just send the response.
            if (rmap_service_module_Power_Request_1_0_deserialize_(&req, static_cast<uint8_t const*>(transfer->payload), &size) >= 0) {
                // I dati e metadati sono direttamente popolati in processRequestGetModuleData
                rmap_service_module_Power_Response_1_0 module_radiation_resp = processRequestGetModuleData(clCanard, &req, (CanParam_t *) param);
                // Serialize and publish the message:
                uint8_t serialized[rmap_service_module_Power_Response_1_0_SERIALIZATION_BUFFER_SIZE_BYTES_] = {0};
                size_t serialized_size = sizeof(serialized);
                const int8_t res = rmap_service_module_Power_Response_1_0_serialize_(&module_radiation_resp, &serialized[0], &serialized_size);
                if (res >= 0) {
    ...
    while (true) {
        ...
        switch (state) {
            ...
            case CAN_STATE_INIT:
                ...
                clCanard.port_id.publisher_module_mppt = (CanardPortID)SUBJECTID_PUBLISH_RMAP;
                #else
                clCanard.port_id.publisher_module_mppt =
                    getModeAccessID(canardClass::Introspection_Port::PublisherSubjectID,
                        "POW.data_and_metadata", rmap_module_Power_1_0_FULL_NAME_AND_VERSION_);
                #endif
                #ifdef USE_PORT_SERVICE_RMAP_FIXED
                clCanard.port_id.service_module_mppt = (CanardPortID)PORT_SERVICE_RMAP;
                #else
                clCanard.port_id.service_module_mppt =
                    getModeAccessID(canardClass::Introspection_Port::ServicePortID,
                        "POW.service_data_and_metadata", rmap_service_module_Power_1_0_FULL_NAME_AND_VERSION_);
                #endif
                ...
                localRegisterAccessLock->Take();
                localRegister->read("rmap.module.POW.metadata.Level.L1", &val);
                localRegisterAccessLock->Give();
                ...
                localRegisterAccessLock->Take();
                localRegister->read("rmap.module.POW.metadata.Level.L2", &val);
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                ...
                localRegisterAccessLock->Take();
                localRegister->read("rmap.module.POW.metadata.Level.LevelType1", &val);
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                ...
                localRegisterAccessLock->Take();
                localRegister->read("rmap.module.POW.metadata.Level.LevelType2", &val);
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                ...
                localRegisterAccessLock->Take();
                localRegister->read("rmap.module.POW.metadata.Timerange.P1", &val);
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                ...
                val.natural16.value.elements[SENSOR_METADATA_DEP] = SENSOR_METADATA_LEVEL_P_IND_DEP;
                localRegisterAccessLock->Take();
                localRegister->read("rmap.module.POW.metadata.Timerange.Pindicator", &val);
                localRegisterAccessLock->Give();
            ...
            case CAN_STATE_SETUP:
                ...
                if (!clCanard.rxSubscribe(CanardTransferKindRequest,
                                        clCanard.port_id.service_module_mppt,
                                        rmap_service_module_Power_Request_1_0_EXTENT_BYTES_,
                                        CANARD_DEFAULT_TRANSFER_ID_TIMEOUT_USEC)) {
                    LOCAL_ASSERT(false);
                }
