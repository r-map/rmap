...
canardClass::canardClass() {
    ...
    port_id.publisher_module_mppt = UINT16_MAX;
    port_id.service_module_mppt = UINT16_MAX;

    publisher_enabled.module_mppt = DEFAULT_PUBLISH_MODULE_DATA;
...
bool canardClass::slave_servicelist_send_message(void)
{
        ...
        (_canard.node_id <= CANARD_NODE_ID_MAX))
    {
        ...
        // Indicate which subjects we publish to. Don't forget to keep this updated if you add new publications!
        {
            ...
            if ((port_id.publisher_module_mppt <= CANARD_SUBJECT_ID_MAX)&&
                (publisher_enabled.module_mppt))
            {
                m.publishers.sparse_list.elements[(*cnt)++].value = port_id.publisher_module_mppt;
            }
        }
...
/// @param  None
/// @return Prossimo transfer_id valido in standard UAVCAN
uint8_t canardClass::next_transfer_id::module_mppt(void) {
    return next_transfer_id::_module_mppt++;
}
