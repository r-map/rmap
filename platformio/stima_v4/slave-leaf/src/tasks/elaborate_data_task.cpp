...
void ElaborateDataTask::make_report (bool is_init, uint16_t report_time_s, uint8_t observation_time_s) {
  ...
  float avg_leaf_s = 0;
  float avg_leaf_o = 0;
  float min_leaf_o = FLT_MAX;
  float max_leaf_o = FLT_MIN;
  ...
  if (report_time_s == 0)
  {
    ...
    report.avg = bufferReadBack<sample_t, uint16_t, rmapdata_t>(&leaf_samples, SAMPLES_COUNT_MAX);
  }
  else
  {
    for (uint16_t i = 0; i < samples_count; i++)
    {
      ...
      if ((ISVALID_RMAPDATA(leaf_s)) && !measures_maintenance)
      {
        ...
        valid_count_leaf_t++;
        #endif
        avg_leaf_s += (float)(((float)leaf_s - avg_leaf_s) / valid_count_leaf_s);
      }
      ...
      if(is_observation) {
        ...
        {
          valid_count_leaf_o++;
          avg_leaf_o += (avg_leaf_s - avg_leaf_o) / valid_count_leaf_o;
          avg_leaf_quality_o += (avg_leaf_quality_s - avg_leaf_quality_o) / valid_count_leaf_o;
          // Elaboration MIN and MAX for observation
          if(avg_leaf_s < min_leaf_o) min_leaf_o = avg_leaf_s;
          if(avg_leaf_s > max_leaf_o) max_leaf_o = avg_leaf_s;
        }
