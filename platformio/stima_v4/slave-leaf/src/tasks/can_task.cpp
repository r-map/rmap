...
void CanTask::prepareSensorsDataValue(uint8_t const sensore, const report_t *report, rmap_module_Leaf_1_0 *rmap_data) {
...
void CanTask::prepareSensorsDataValue(uint8_t const sensore, const report_t *report, rmap_service_module_Leaf_Response_1_0 *rmap_data) {
    // Inserisco i dati reali
    switch (sensore) {
        case canardClass::Sensor_Type::bft:
            // Prepara i dati SMP (Sample)
...
void CanTask::publish_rmap_data(canardClass &clCanard, CanParam_t *param) {
    ...
    if ((!clCanard.is_canard_node_anonymous()) &&
        (clCanard.publisher_enabled.module_leaf) &&
        (clCanard.port_id.publisher_module_leaf <= CANARD_SUBJECT_ID_MAX)) {
        rmap_module_Leaf_1_0 module_leaf_msg = {0};
        ...
        clCanard.module_leaf.BFT.metadata.timerange.P2 = request_data.report_time_s;
        ...
        // Metadata
        module_leaf_msg.BFT.metadata = clCanard.module_leaf.BFT.metadata;

        // Serialize and publish the message:
        uint8_t serialized[rmap_module_Leaf_1_0_SERIALIZATION_BUFFER_SIZE_BYTES_] = {0};
        size_t serialized_size = sizeof(serialized);
        const int8_t err = rmap_module_Leaf_1_0_serialize_(&module_leaf_msg, &serialized[0], &serialized_size);
        LOCAL_ASSERT(err >= 0);
        if (err >= 0) {
            const CanardTransferMetadata meta = {
                .priority = CanardPrioritySlow,
                .transfer_kind = CanardTransferKindMessage,
                .port_id = clCanard.port_id.publisher_module_leaf,
                .remote_node_id = CANARD_NODE_ID_UNSET,
                .transfer_id = (CanardTransferID)(clCanard.next_transfer_id.module_leaf()),  // Increment!
            };
...
uavcan_node_ExecuteCommand_Response_1_1 CanTask::processRequestExecuteCommand(canardClass &clCanard, const uavcan_node_ExecuteCommand_Request_1_1* req,
    ...
    switch (req->command)
    {
        ...
        case canardClass::Command_Private::enable_publish_rmap:
        {
            ...
            clCanard.publisher_enabled.module_leaf = true;
        ...
        case canardClass::Command_Private::disable_publish_rmap:
        {
            ...
            clCanard.publisher_enabled.module_leaf = false;
...
rmap_service_module_Leaf_Response_1_0 CanTask::processRequestGetModuleData(canardClass &clCanard, rmap_service_module_Leaf_Request_1_0* req, CanParam_t *param) {
    rmap_service_module_Leaf_Response_1_0 resp = {0};
    ...
    switch (req->parameter.command) {
        ...
        case rmap_service_setmode_1_0_get_last:     // saturated uint3 get_last = 2
          ...
          // Esiste il solo caso ::bft sia per get istant che archivio
          prepareSensorsDataValue(canardClass::Sensor_Type::bft, &report_srv, &resp);
          break;
    ...
    resp.BFT.metadata = clCanard.module_leaf.BFT.metadata;
...
void CanTask::processReceivedTransfer(canardClass &clCanard, const CanardRxTransfer* const transfer, void *param) {
    ...
    else if (transfer->metadata.transfer_kind == CanardTransferKindRequest)
    {
        ...
        if (transfer->metadata.port_id == clCanard.port_id.service_module_leaf) {
            // Richiesta ai dati e metodi di sensor drive
            rmap_service_module_Leaf_Request_1_0 req = {0};
            size_t size = transfer->payload_size;
            TRACE_INFO_F(F("<<-- Ricevuto richiesta dati da master\r\n"));
            // The request object is empty so we don't bother deserializing it. Just send the response.
            if (rmap_service_module_Leaf_Request_1_0_deserialize_(&req, static_cast<uint8_t const*>(transfer->payload), &size) >= 0) {
                // I dati e metadati sono direttamente popolati in processRequestGetModuleData
                rmap_service_module_Leaf_Response_1_0 module_leaf_resp = processRequestGetModuleData(clCanard, &req, (CanParam_t *) param);
                // Serialize and publish the message:
                uint8_t serialized[rmap_service_module_Leaf_Response_1_0_SERIALIZATION_BUFFER_SIZE_BYTES_] = {0};
                size_t serialized_size = sizeof(serialized);
                const int8_t res = rmap_service_module_Leaf_Response_1_0_serialize_(&module_leaf_resp, &serialized[0], &serialized_size);
                if (res >= 0) {
    ...
    while (true) {
        ...
        switch (state) {
            ...
            case CAN_STATE_INIT:
                ...
                clCanard.port_id.publisher_module_leaf = (CanardPortID)SUBJECTID_PUBLISH_RMAP;
                #else
                clCanard.port_id.publisher_module_leaf =
                ...
                clCanard.port_id.service_module_leaf = (CanardPortID)PORT_SERVICE_RMAP;
                #else
                clCanard.port_id.service_module_leaf =
                    getModeAccessID(canardClass::Introspection_Port::ServicePortID,
                        REGISTER_DATA_SERVICE, rmap_service_module_Leaf_1_0_FULL_NAME_AND_VERSION_);
                #endif
                ...
                if(clCanard.port_id.service_module_leaf != UINT16_MAX)
                ...
                clCanard.module_leaf.BFT.metadata.level.L1.value = val.natural16.value.elements[SENSOR_METADATA_BFT];
                // *********************************** L2 *********************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                for(uint8_t id=0; id<SENSOR_METADATA_COUNT; id++) {
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_leaf.BFT.metadata.level.L2.value = val.natural16.value.elements[SENSOR_METADATA_BFT];
                // ******************************* LevelType1 *****************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                for(uint8_t id=0; id<SENSOR_METADATA_COUNT; id++) {
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_leaf.BFT.metadata.level.LevelType1.value = val.natural16.value.elements[SENSOR_METADATA_BFT];
                // ******************************* LevelType2 *****************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                for(uint8_t id=0; id<SENSOR_METADATA_COUNT; id++) {
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_leaf.BFT.metadata.level.LevelType2.value = val.natural16.value.elements[SENSOR_METADATA_BFT];
                // *********************************** P1 *********************************************
                uavcan_register_Value_1_0_select_natural16_(&val);
                val.natural16.value.count = SENSOR_METADATA_COUNT;
                for(uint8_t id=0; id<SENSOR_METADATA_COUNT; id++) {
                ...
                localRegisterAccessLock->Give();
                LOCAL_ASSERT(uavcan_register_Value_1_0_is_natural16_(&val) && (val.natural16.value.count == SENSOR_METADATA_COUNT));
                clCanard.module_leaf.BFT.metadata.timerange.P1.value = val.natural16.value.elements[SENSOR_METADATA_BFT];
                // *********************************** P2 *********************************************
                // P2 Non memorizzato sul modulo, parametro dipendente dall'acquisizione locale
                clCanard.module_leaf.BFT.metadata.timerange.P2 = 0;
                // *********************************** P2 *********************************************
                ...
                val.natural8.value.elements[SENSOR_METADATA_BFT] = SENSOR_METADATA_LEVEL_P_IND_BFT;
                localRegisterAccessLock->Take();
                ...
                clCanard.module_leaf.BFT.metadata.timerange.Pindicator.value = val.natural8.value.elements[SENSOR_METADATA_BFT];
            ...
            case CAN_STATE_SETUP:
                ...
                if (!clCanard.rxSubscribe(CanardTransferKindRequest,
                                        clCanard.port_id.service_module_leaf,
                                        rmap_service_module_Leaf_Request_1_0_EXTENT_BYTES_,
