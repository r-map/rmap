...
// To Optimize pagebuffer + address + write_flash_page (also in stk500_function)
// variables global (FAT && Buffer)
FATFS Fatfs;							// Petit-FatFs work area
uint8_t pagebuffer[SPM_PAGESIZE];	// Page data buffer
#if BOOT_ADR > 0xFFFF
  uint32_t address;
#else
  uint16_t address;
...
// Write Flash Memory (Update Program from Boot)
...
static uint8_t pagecmp()
{
	...
	for (i = 0; i < SPM_PAGESIZE; i++) {
		if (PGM_READ_BYTE(address + i) != pagebuffer[i]) {
			// Flash are different (Block is ready to Program)
			#if (UART_REPORT)
			UART_puthex32(address);
			UART_puts(PSTR("->"));
			UART_newline();
			#endif
			return 1;
...
void doFlash() {
  ...
  for(i=0;i<10;i++) {
	...
	for (address = 0; address < BOOT_ADR; address += SPM_PAGESIZE) {	/* Update all application pages */
		...
		pf_read(pagebuffer, SPM_PAGESIZE);				/* Load a page data without control readbyte */
...
void checkFile() {
  ...
  doFlash();
	...
	#if USE_LED
	  uint8_t i;
		// SUCCESS FLASH WILDLY for 2 secs
    for(i=0;i<40;i++) {
			led_power_toggle();
      watchdogReset();
			_delay_ms(50);
