...
void GPS_SerialInit(gps_data_t& data) {
  data.logger->notice(F("start GPS serial init"));  
  Serial2.begin(GPS_SERIAL_SPEED);
  Serial.setTimeout(100);
  data.logger->notice(F("end GPS serial init"));
}
...
void doSerialNmea(gps_data_t& data){
  ...
  if (len > 0) data.logger->verbose("message: %s", message);
  ...
  if (gps_gps.valid){
    data.logger->notice(F("RMC latitude : %5"), gps_mgps.rmc.dms.latitude);
    data.logger->notice(F("RMC longitude: %5"), gps_mgps.rmc.dms.longitude);
    //data.logger->notice(F("GGA latitude : %5"), gps_mgps.gga.dms.latitude);
    //data.logger->notice(F("GGA longitude: %5"), gps_mgps.gga.dms.longitude);
    //data.logger->notice(F("GLL latitude : %5"), gps_mgps.gll.dms.latitude);
    //data.logger->notice(F("GLL longitude: %5"), gps_mgps.gll.dms.longitude);
    data.logger->notice(F("RMC datetime: %d %d %d %d %d %d"), gps_mgps.rmc.time.year, gps_mgps.rmc.time.mon, gps_mgps.rmc.time.day,
...
void gpsThread::Run() {
  ...
  GPS_SerialInit(data);
  data.logger->notice(F("Listening on GPS serial port"));
