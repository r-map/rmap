...
void print_configuration() {
   ...
   LOGN(F("--> How much Hg/m^2 of rain for one tip of tipping bucket rain gauge: %d"), configuration.rain_for_tip);
   #endif
}
...
void sensors_reading_task () {
   ...
   switch (sensors_reading_state) {
	...
	if (!sensors[i]->isSetted()) {
	  ...
	  addValue<sample_t, uint16_t, float>(&temperature_samples, SAMPLES_COUNT, UINT32_MAX);
	  addValue<sample_t, uint16_t, float>(&humidity_samples, SAMPLES_COUNT, UINT32_MAX);
        ...
        //! read sensor value
	{
	  ...
	  if (error) {
	    ...
	    unsigned long int value = doc["B12101"] | UINT32_MAX;
            addValue<sample_t, uint16_t, float>(&temperature_samples, SAMPLES_COUNT, value);
	    
	    value = doc["B13003"] | UINT32_MAX;
            addValue<sample_t, uint16_t, float>(&humidity_samples, SAMPLES_COUNT, value);
	  }
	}
