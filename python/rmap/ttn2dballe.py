    ...
    for t in self.map:
        if paho.topic_matches_sub(t, msg.topic):
            ...
            try:
                ...
                mqtt=rmapmqtt.rmapmqtt(user=user,username=rmap.settings.mqttuser,password=rmap.settings.mqttpassword,lon=mystation.lon,lat=mystation.lat,network="fixed",host="localhost",prefix="sample",maintprefix="maint",logfunc=logging.debug)
