...
static TaskData_t inputTaskData;
...
static void inputTask() {
  ...
  for (;;) {
    ...
    event_wait(serialinputEvt); 
    //debug("input");
    uint8_t command=input_getcommand();
    
    if (command >0){
      // arrow up pressed
      debug("arrow up pressed");
      event_ISR_signal(prevChEvt);
    }else if (command < 0){
      debug("arrow down pressed");
      event_ISR_signal(nextChEvt);
    }    
...
static void displayTask() {
  ...
  msg_post_every(displayTaskId, msg, 40);  //refresh display every 40 ticks
