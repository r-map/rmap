...
void received_data(){
  ...
  while (new_data == 0 && (millis()-startt) < 3000) delay(10);
...
void request_data(){
  ...
  uint8_t a=1;
  while  (Wire.requestavailable()>0) {   // slave may send less than requested
    LOGN(F("requested %d: %d" CR),a++,Wire.requestread());
  }
}
