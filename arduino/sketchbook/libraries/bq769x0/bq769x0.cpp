...
int bq769x0::checkStatus()
{
  ...
  if (alertInterruptFlag == false && errorStatus == 0) 
  {
    ...
    if (sys_stat.bits.CC_READY == 1) 
	{
      ...
      //updateCurrent(false);  // automatically clears CC ready flag		
	  updateCurrent(true);  // automatically clears CC ready flag			>>>>>>>>>>>>>> Portare a FALSE se si vuole gestire il bit del registro esternamente
    }
    ...
    if (sys_stat.regByte & B00111111)
    {
      ...
      unsigned int secSinceInterrupt = (millis() - interruptTimestamp) / 1000;
...
bool bq769x0::enableCharging()
{
	...
	if (checkStatus() == 0 && cellVoltages[idCellMaxVoltage] < maxCellVoltage && temperatures[0] > minCellTempCharge && temperatures[0] < maxCellTempCharge )
...
{
  if (checkStatus() == 0 && cellVoltages[idCellMinVoltage] > minCellVoltage && temperatures[0] < maxCellTempDischarge && temperatures[0] > minCellTempDischarge)
  {
    byte sys_ctrl2;
    sys_ctrl2 = readRegister(SYS_CTRL2);
...
void bq769x0::writeRegisterBit(byte address, int bit, int valore_bit)
{
  int data;  
  data=readRegister(address);	//leggo il valore attuale
...
int bq769x0::readRegisterBit(byte address, int bit)
{  
  Wire.beginTransmission(I2CAddress);
  Wire.write(address);
  Wire.endTransmission();
