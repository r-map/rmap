...
PubSubClient mqttclient("rmap.cc", 1883, espClient);
#endif
...
void wifiConfig() {
	...
	debug_out(F("AP ID: RMAP-"), DEBUG_MIN_INFO, 0);
...
void connectWifi() {
	...
	debug_out("", DEBUG_MIN_INFO, 1);
	if (WiFi.status() != WL_CONNECTED) {
		display_debug("AP ID: RMAP-" + esp_chipid + " - IP: 192.168.4.1");
		wifiConfig();
		if (WiFi.status() != WL_CONNECTED) {
			retry_count = 0;
			while ((WiFi.status() != WL_CONNECTED) && (retry_count < 20)) {
...
String sensorSDS() {
	...
	if (long(act_milli - starttime) < (long(sending_intervall_ms) - long(warmup_time_SDS_ms + reading_time_SDS_ms))) {
		if (is_SDS_running) {
		        debug_out(F("stop_SDS"), DEBUG_MED_INFO, 0);
			stop_SDS();
		}
	} else {
		if (! is_SDS_running) {
		        debug_out(F("start_SDS"), DEBUG_MED_INFO, 0);
			start_SDS();
		}
...
//			"aa" = 170, "ab" = 171, "c0" = 192
	...
	if (send_now) {
		last_value_SDS_P1 = "";
		last_value_SDS_P2 = "";
		debug_out("sds_val_count:  " + String(sds_val_count), DEBUG_MAX_INFO, 1);
		if (sds_val_count > 2) {
...
void setup() {
	...
	if (sds_read) {
		debug_out(F("Stop SDS011..."), DEBUG_MIN_INFO, 1);
		stop_SDS();
	}
	if (pms24_read || pms32_read) {
