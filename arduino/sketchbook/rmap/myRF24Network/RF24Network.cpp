...
void RF24Network::update(void)
{
  ...
  while ( radio.isValid() && radio.available(&pipe_num) )
  {
	...
	{
	   // char tmp[33];
	   // memcpy(tmp,frame_buffer+ sizeof(RF24NetworkHeader),frame_size- sizeof(RF24NetworkHeader));
	   // memcpy(tmp+frame_size- sizeof(RF24NetworkHeader),"\0",1);
	   // Serial.println(tmp);
	   // Serial.println("decode aeslib");

	  IF_SERIAL_DEBUG(printf_P(PSTR("decode aeslib\n\r")));
      ...
      if (key)
	{
	   ...
	   // memcpy(tmp+frame_size-sizeof(RF24NetworkHeader),"\0",1);
	   // Serial.println(tmp);
	   // Serial.println("decode aesgladman");
	  
	  IF_SERIAL_DEBUG(printf_P(PSTR("decode aesgladman\n\r")));

	  uint8_t my_iv[2];
	  for (byte i = 0 ; i < 2 ; i++)
	    iv[i] = my_iv[i] ;
	  aes.cbc_decrypt (frame_buffer, frame_buffer, 2, my_iv) ;
      ...
      IF_SERIAL_DEBUG(printf_P(PSTR("Header:%s\n\r"),header.toString()));
...
size_t RF24Network::readmulti(RF24NetworkHeader& header,void* message, size_t maxlen)
{
  ...
  IF_SERIAL_DEBUG(printf_P(PSTR("readmulti start at %lu\n\r"),start_at));
  ...
  while ( ( millis() - start_at) < timeout ){
    ...
    while ( availablefifo() ){
      ...
      IF_SERIAL_DEBUG(printf_P(PSTR("multi header : %s\n\r"),header.toString()));
      if (header.type == 100 && myid == 0 ) {
      	myid=header.id;
      	npacket=header.reserved;
	nextpacket=npacket;
	start_at = millis();
	IF_SERIAL_DEBUG(printf_P(PSTR("start at %lu\n\r"),start_at));
      }
	...
	IF_SERIAL_DEBUG(printf_P(PSTR("maxlen %u\n\r"),maxlen));
	IF_SERIAL_DEBUG(printf_P(PSTR("payload_size %u\n\r"),PAYLOAD_SIZE));
	IF_SERIAL_DEBUG(printf_P(PSTR("%u\n\r"),(min(maxlen-((npacket-header.reserved  )*PAYLOAD_SIZE),PAYLOAD_SIZE))));
	IF_SERIAL_DEBUG(printf_P(PSTR("bufsize : %u\n\r"),bufsize));
	IF_SERIAL_DEBUG(printf_P(PSTR("npacket : %u\n\r"),npacket));
	IF_SERIAL_DEBUG(printf_P(PSTR("npack   : %u\n\r"),npacket-header.reserved));

	//int len = readfifo(header,from,bufsize);
      	//lentot += len ;
	//Serial.print("< lentot  : "); Serial.println(lentot);

	lentot += readfifo(header,from,bufsize);

	//char tmp[25];
	//memcpy(tmp,(char*)from,len);
	//memcpy(tmp+24,"\0",1);
	//Serial.print("< from    : "); Serial.println(tmp);

	if (!availablefifo()) next_frame = frame_queue;

	IF_SERIAL_DEBUG(printf_P(PSTR("nextpacket : %u\n\r"),nextpacket));
	...
	// skip packet
	IF_SERIAL_DEBUG(printf_P(PSTR("Skip\n\r")));
...
bool RF24Network::writemulti(RF24NetworkHeader& header, const void* message, size_t len)
{
  ...
  while (npack < numpack)
	...
	}else
	{
      ...
      IF_SERIAL_DEBUG(printf_P(PSTR("header: %s\n\r") ,header.toString()));
      IF_SERIAL_DEBUG(printf_P(PSTR("lenlen: %u\n\r"), lenlen));
      IF_SERIAL_DEBUG(printf_P(PSTR("npack : %u\n\r"),npack));
      IF_SERIAL_DEBUG(printf_P(PSTR("point : %u\n\r"),(npack*PAYLOAD_SIZE)));

      // char tmp[25];
      // memcpy(tmp,(char*)message+(npack*PAYLOAD_SIZE),lenlen);
      // memcpy(tmp+24,"\0",1);
      // Serial.print("> messa: ");Serial.println(tmp);
...
bool RF24Network::write_to_pipe( uint16_t node, uint8_t pipe )
{
  ...
  uint64_t out_pipe = pipe_address( node, pipe );
   ...
   if (key)
     {
        // char tmp[frame_size+1];
        // memcpy(tmp,frame_buffer+ sizeof(RF24NetworkHeader),frame_size- sizeof(RF24NetworkHeader));
        // memcpy(tmp+frame_size-sizeof(RF24NetworkHeader),"\0",1);
        // Serial.println(tmp);
        // Serial.print("encode aeslib ");
        // Serial.println(frame_size);

       IF_SERIAL_DEBUG(printf_P(PSTR("encode aeslib\n\r")));
       aes128_cbc_enc(key, iv, frame_buffer, frame_size);
   ...
   if (key)
     {
        ...
        // memcpy(tmp+frame_size-sizeof(RF24NetworkHeader),"\0",1);
        // Serial.println(tmp);
        // Serial.print("encode aesgladman");
        // Serial.println(frame_size);

       IF_SERIAL_DEBUG(printf_P(PSTR("encode aesgladman\n\r")));
       uint8_t my_iv[2];
       for (byte i = 0 ; i < 2 ; i++)
	 iv[i] = my_iv[i] ;
       aes.cbc_encrypt (frame_buffer, frame_buffer, 2, my_iv) ;
